...
Vertex = Proto.clone().newSlots({
	...
	*/	 
	...
	{
		//writeln("request.headers['cookie']: " + request.headers['cookie']);
	...
	writeResultOnResponseWithStatus: function(results, response, status)
	{
		...
		response.writeHead(status, 
			{ 
				...
				//"Set-Cookie": "username=guest; domain=.locahost;"
			}
		);
		response.write(body);
		response.end();
