	...
	simpleHandleRequest: function(request, response)
	{
		...
		*/
		...
		try
		{
			...
			//writeln("request.headers = " + JSON.stringify(request.headers))
			//writeln("request.headers['cookie'] = " + JSON.stringify(request.headers['cookie']))
			//writeln("this.user().username() = " + this.user().username());
			//writeln("this.user().isValid() = " + this.user().isValid());
		}
		catch (e)
		{
